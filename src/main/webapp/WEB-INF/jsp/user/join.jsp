...
.header h1{color:#F5E6C8;font-size:22px;}
...
.join-card{background:#FFFDF8;width:100%;max-width:460px;padding:40px;border-radius:12px;box-shadow:0 4px 20px rgba(92,61,46,0.15);}
.join-card h2{color:#5C3D2E;text-align:center;margin-bottom:24px;font-size:22px;border-bottom:2px solid #A0522D;padding-bottom:10px;}
...
<div class="join-container">
  <div class="join-card">
    ...
    <hr class="divider">
    <div class="join-footer">이미 회원이신가요? <a href="${pageContext.request.contextPath}/user/loginView.do?returnUrl=${returnUrl}">로그인 하기</a></div>
    <div class="join-footer" style="margin-top:8px;"><a href="${pageContext.request.contextPath}/book/bookList.do">← 메인으로 돌아가기</a></div>
  </div>
</div>
<div class="footer">2026 폴리 인공지능 도서관</div>
<script>
var CP='${pageContext.request.contextPath}';
var idChecked=false,pwMatched=false;
function checkId(){
  var id=document.getElementById('memberId').value.trim();
  var msg=document.getElementById('idMsg'),inp=document.getElementById('memberId');
  if(!id){msg.textContent='아이디를 입력해주세요.';msg.className='check-msg err';return;}
  var x=new XMLHttpRequest();x.open('GET',CP+'/user/checkId.do?memberId='+encodeURIComponent(id),true);
  x.onload=function(){
    if(x.responseText==='AVAILABLE'){msg.textContent='✔ 사용 가능한 아이디입니다.';msg.className='check-msg ok';inp.classList.add('ok');inp.classList.remove('err');idChecked=true;}
    else{msg.textContent='✗ 이미 사용 중인 아이디입니다.';msg.className='check-msg err';inp.classList.add('err');inp.classList.remove('ok');idChecked=false;}
  };x.send();
}
function resetIdCheck(){idChecked=false;document.getElementById('idMsg').textContent='';document.getElementById('idMsg').className='check-msg';document.getElementById('memberId').classList.remove('ok','err');}
function checkPwMatch(){
  var pw=document.getElementById('password').value,pw2=document.getElementById('passwordConfirm').value;
  var msg=document.getElementById('pwMsg'),inp=document.getElementById('passwordConfirm');
  if(!pw2){msg.textContent='';pwMatched=false;return;}
  if(pw===pw2){msg.textContent='✔ 비밀번호가 일치합니다.';msg.className='check-msg ok';inp.classList.add('ok');inp.classList.remove('err');pwMatched=true;}
  else{msg.textContent='✗ 비밀번호가 일치하지 않습니다.';msg.className='check-msg err';inp.classList.add('err');inp.classList.remove('ok');pwMatched=false;}
}
function validateForm(){
  if(!idChecked){alert('아이디 중복 확인을 해주세요.');return false;}
  if(!pwMatched){alert('비밀번호가 일치하지 않습니다.');return false;}
  return true;
}
</script>
</body>
